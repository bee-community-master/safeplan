...
export function ReviewCards({ caseId }: { caseId: string }) {
  ...
  return (
    <section className="rounded-3xl bg-white p-6 shadow-sm">
      <h1 className="text-3xl font-bold">자료 카드 검토</h1>
      <p className="mt-3 text-stone-700">요약, 태그, 날짜, 인물 정보는 자동 정리 초안입니다. 리포트에는 사용자가 확인하고 포함한 카드만 들어갑니다.</p>
      <div className="mt-6 space-y-5">
        {cards.map((card, index) => (
          <article key={card.id} className="rounded-2xl border border-stone-200 p-4" data-testid="evidence-card">
            <p className="text-sm font-semibold text-clay">자료 {index + 1} · 추출 신뢰도 {card.confidenceLevel}단계 · {confidencePolicyLabel(card.confidenceLevel)}</p>
      ...
      {cards.length === 0 && <p className="mt-4 rounded-xl bg-amber-50 p-4">아직 준비된 자료 카드가 없습니다. 업로드 화면에서 자료 정리를 시작하세요.</p>}
